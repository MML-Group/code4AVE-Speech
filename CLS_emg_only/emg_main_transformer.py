...
from emg_dataset_transformer import *
...
def test_adam(args, use_gpu):
    ...
    save_path = '/ai/exp/fusion_baseline_231205_new/emg_only/' + args.mode + '_every_frame'

    if not os.path.isdir(save_path):
        os.mkdir(save_path)
    # logging info
    filename = save_path+'/'+args.mode+'_'+ 'emg_' + str(args.lr) + '.txt'
    logger_name = "mylog"
    ...
    model = emg_model(mode=args.mode, inputDim=256, hiddenDim=512, nClasses=args.nClasses, frameLen=40, every_frame=args.every_frame)
    # reload model
    ...
    scheduler = AdjustLR(optimizer, [args.lr], sleep_epochs=35, half=5, verbose=1)
    if args.test:
