...
from lip_dataset_transformer import *
...
def train_test(model, dset_loaders, criterion, epoch, phase, optimizer, args, logger, use_gpu, save_path):
    ...
    if phase == 'train':
        ...
        torch.save(model.state_dict(), save_path+'/'+args.mode+'_'+ 'lip_transformer_1207_' +str(epoch+1)+'.pt')
        return model
    if phase == 'val' or phase == 'test':
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(dset_loaders[phase]):
...
def test_adam(args, use_gpu):
    ...
    save_path = '/ai/exp2/fusion_baseline_231205_new/video_only/' + args.mode + '_every_frame'

    if not os.path.isdir(save_path):
        os.mkdir(save_path)
    # logging info
    filename = save_path+'/'+args.mode+'_'+ 'lip_transformer1207_' +str(args.lr)+'.txt'
    logger_name = "mylog"
    ...
    # model = lipreading(d_obs=512, d_model=512, d_class=101, d_k=128, d_inner=2048, n_head=4, n_layers=3) # 0324
    model = lipreading(d_obs=512, d_model=512, d_class=101, d_k=256, d_inner=1024, n_head=2, n_layers=2)
    # reload model
    ...
    scheduler = AdjustLR(optimizer, [args.lr], sleep_epochs=10, half=5, verbose=1)
    if args.test:
