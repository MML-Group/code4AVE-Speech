...
from audio_only.model import lipreading as audio_model_sr
from video_only.lip_model import lipreading as video_model_sr
from emg_only.emg_model_1D import emg_model as emg_model_sr
...
def train_test(audio_model, lip_model, emg_model, concat_model, dset_loaders, criterion, epoch, phase, optimizer, args, logger, use_gpu, save_path):
    ...
    if phase == 'train':
        for batch_idx, (audio, emg, lip, targets) in enumerate(dset_loaders[phase]):
            # emg = emg.reshape(emg.shape[0], emg.shape[2], emg.shape[1])
        ...
        torch.save(concat_model.state_dict(), save_path+'/BGRU_1D_'+args.mode+'_'+str(epoch+1)+'.pt')
...
def test_adam(args, use_gpu):
    ...
    if not os.path.isdir(save_path):
        os.mkdir(save_path)
    # logging info
    filename = save_path+'/'+args.mode+'BGRU_1D_'+str(args.lr)+'.txt'
    logger_name = "mylog"
...
def main():
    ...
    parser.add_argument('--audio-path', default='audio_only/finetuneGRU_67.pt', help='path to pre-trained audio model')
    parser.add_argument('--lip-path', default='video_only/finetuneGRU_every_frame/finetuneGRU_0906_22.pt', help='path to pre-trained lip model')
    parser.add_argument('--emg-path', default='emg_only/finetuneGRU_every_frame/finetuneGRU_266.pt', help='path to pre-trained emg model')
    parser.add_argument('--concat-path', default='finetuneGRU_every_frame/BGRU_1D_finetuneGRU_3.pt', help='path to pre-trained concat model')
