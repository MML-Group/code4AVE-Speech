...
from audio_model_BGRU_CSR import audiosr as audio_model_sr
from audio_dataset_CSR import *
...
def train_test(model, dset_loaders, criterion, epoch, phase, optimizer, args, logger, use_gpu, save_path):
    ...
    if phase == 'train':
        for batch_idx, (inputs, annos, anno_len, audio_len) in enumerate(dset_loaders[phase]):
            inputs = inputs.float()
            inputs, annos = Variable(inputs.cuda()), Variable(annos.cuda())
            inputs, annos, anno_len, audio_len = inputs.cuda(), annos.cuda(), anno_len.cuda(), audio_len.cuda()
            outputs = model(inputs)
            loss = criterion(outputs.transpose(0, 1).log_softmax(-1), annos, audio_len.view(-1), anno_len.view(-1))
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        logger.info('{} Epoch:\t{:2}\tLoss: {:.4f}\tWER:{:.4f}'.format(
            ...
            np.array(train_wer).mean()))
        torch.save(model.state_dict(), save_path+'/'+'audio_CSR_'+str(epoch+1)+'.pt')
        return model
    if phase == 'val' or phase == 'test':
        with torch.no_grad():
            for batch_idx, (inputs, annos, anno_len, audio_len) in enumerate(dset_loaders[phase]):
...
def test_adam(args, use_gpu):
    save_path = '/ai/mm/audio_only'
    if not os.path.isdir(save_path):
        os.mkdir(save_path)
    # logging info
    filename = save_path+'/'+args.mode+'_'+ 'audio_CSR_' +str(args.lr)+'.txt'
    logger_name = "mylog"
    ...
    model = audio_model_sr(mode=args.mode, inputDim=512, hiddenDim=512, nClasses=args.nClasses, frameLen=60, every_frame=args.every_frame)
    # reload model
...
def main():
    ...
    parser.add_argument('--path', default='/ai/mm/audio_only/audio_CSR_WER_1.70.pt', help='path to model')
    ...
    parser.add_argument('--batch-size', default=128, type=int, help='mini-batch size (default: 36)')
    parser.add_argument('--workers', default=0, type=int, help='number of data loading workers (default: 4)')
    parser.add_argument('--epochs', default=60, type=int, help='number of total epochs')
