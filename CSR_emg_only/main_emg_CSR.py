...
def train_test(model, dset_loaders, criterion, epoch, phase, optimizer, args, logger, use_gpu, save_path):
    ...
    if phase == 'train':
        for batch_idx, (inputs, annos, anno_len, emg_len) in enumerate(dset_loaders[phase]):
            ...
            inputs, annos, anno_len, emg_len = inputs.cuda(), annos.cuda(), anno_len.cuda(), emg_len.cuda()
            outputs = model(inputs)
            loss = criterion(outputs.transpose(0, 1).log_softmax(-1), annos, emg_len.view(-1), anno_len.view(-1))
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        logger.info('{} Epoch:\t{:2}\tLoss: {:.4f}\tWER:{:.4f}'.format(
            ...
            np.array(train_wer).mean()))
        torch.save(model.state_dict(), save_path+'/'+'emg_CSR_'+str(epoch+1)+'.pt')
        return model
    if phase == 'val' or phase == 'test':
        with torch.no_grad():
            for batch_idx, (inputs, annos, anno_len, emg_len) in enumerate(dset_loaders[phase]):
...
def test_adam(args, use_gpu):
    save_path = '/ai/mm/emg_only'
    if not os.path.isdir(save_path):
        os.mkdir(save_path)
    # logging info
    filename = save_path+'/'+args.mode+'_'+ 'emg_CSR_' +str(args.lr)+'.txt'
    logger_name = "mylog"
    ...
    model = emg_model_sr(mode=args.mode, inputDim=256, hiddenDim=512, nClasses=args.nClasses, frameLen=40, every_frame=args.every_frame)
    # reload model
...
def main():
    ...
    parser.add_argument('--batch-size', default=128, type=int, help='mini-batch size (default: 36)') ## 128
