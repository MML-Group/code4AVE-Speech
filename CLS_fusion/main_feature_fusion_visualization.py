...
def confusion_matrix(preds, labels, conf_matrix):       
    for t, p in zip(labels, preds):  # cm updated
        conf_matrix[t, p] += 1
    return conf_matrix
...
def train_test(audio_model, lip_model, emg_model, concat_model, dset_loaders, criterion, epoch, phase, optimizer, args, logger, use_gpu, save_path):
    ...
    conf_matrix = torch.zeros(101, 101)
    ...
    np.save('conf_matrix_fusion_0327_-10dB.npy', conf_matrix)
    corrects=conf_matrix.diagonal(offset=0)#抽取对角线的每种分类的识别正确个数
    per_kinds=conf_matrix.sum(axis=1)#抽取每个分类数据总的测试条数

    print("每种指令总个数：", per_kinds)
    print("每种指令预测正确的个数：", corrects)
    print("每种指令的识别准确率为：{0}".format([rate*100 for rate in corrects/per_kinds]))
...
def main():
    ...
    parser.add_argument('--emg-path', default='emg_only/finetuneGRU_every_frame/finetuneGRU_245.pt', help='path to pre-trained emg model')
    parser.add_argument('--concat-path', default='finetuneGRU_every_frame/BGRU_finetuneGRU_3.pt', help='path to pre-trained concat model')
