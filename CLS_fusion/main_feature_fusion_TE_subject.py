...
from dataset_TE_subject import *
...
from concat_model_trans import TRANSFORMERConcat as concat_model_sr
...
def train_test(audio_model, lip_model, emg_model, concat_model, dset_loaders, criterion, epoch, phase, optimizer, args, logger, use_gpu, save_path):
    ...
    if phase == 'val' or phase == 'test':
        ...
        with torch.no_grad():
            for batch_idx, (audio, emg, lip, targets) in enumerate(dset_loaders):
...
def test_adam(args, use_gpu):
    ...
    if not os.path.isdir(save_path):
        os.mkdir(save_path)
    # logging info
    filename = save_path+'/'+args.mode+'Transformer_'+str(args.lr)+'.txt'
    logger_name = "mylog"
...
def main():
    ...
    parser.add_argument('--audio-path', default='/ai/benchmark3/fusion_baseline_231205_new/audio_only/finetuneGRU_67.pt', help='path to pre-trained audio model')
    parser.add_argument('--lip-path', default='/ai/benchmark3/fusion_baseline_231205_new/video_only/finetuneGRU_every_frame/finetuneGRU_0906_22.pt', help='path to pre-trained lip model')
    parser.add_argument('--emg-path', default='/ai/benchmark3/fusion_baseline_231205_new/emg_only/finetuneGRU_every_frame/finetuneGRU_245.pt', help='path to pre-trained emg model')
    parser.add_argument('--concat-path', default='/ai/benchmark3/fusion_baseline_231205_new/finetuneGRU_every_frame/Transformer_finetuneGRU_2.pt', help='path to pre-trained concat model')
